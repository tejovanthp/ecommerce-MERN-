    ...
    return (
      <div className="max-w-4xl mx-auto py-12 px-4 animate-in fade-in slide-in-from-bottom-5 duration-500">
        <button 
          ...
          className="flex items-center text-slate-400 hover:text-indigo-600 font-black text-xs uppercase tracking-widest mb-12 transition-colors group"
        >
          <i className="fa-solid fa-arrow-left mr-3 group-hover:-translate-x-2 transition-transform"></i> 
          Back to Order Archives
        </button>

        <div className="bg-white dark:bg-slate-900 rounded-[3.5rem] border border-indigo-50 dark:border-white/5 shadow-2xl overflow-hidden">
          <div className="bg-indigo-700 p-12 text-white relative overflow-hidden">
            ...
            <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
               <div>
                  ...
                  <p className="text-indigo-100/80 font-bold text-sm uppercase tracking-widest">Order Ref: #{trackingOrder.id.split('-')[1]}</p>
               </div>
               <div className="bg-white/10 backdrop-blur-md px-6 py-3 rounded-2xl border border-white/20">
                  <p className="text-[10px] font-black uppercase tracking-widest text-indigo-100 mb-1">Estimated Arrival</p>
          ...
          <div className="p-12 space-y-16">
             ...
             <div className="space-y-10">
                ...
                <div className="relative pt-6">
                  ...
                  {/* The Progress Fill */}
                  <div 
                    className="absolute top-1/2 left-0 h-1.5 bg-indigo-600 rounded-full -translate-y-1/2 transition-all duration-1000 ease-out shadow-[0_0_15px_rgba(79,70,229,0.5)]"
                      ...
                      return (
                        <div key={idx} className="flex flex-col items-center">
                          <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-sm shadow-lg transition-all duration-500 z-10 ${
                            active ? 'bg-indigo-600 text-white border-2 border-white dark:border-slate-900' : 'bg-slate-50 dark:bg-slate-800 text-slate-300 border-2 border-slate-100 dark:border-white/5'
                          } ${isCurrent && trackingOrder.status !== 'DELIVERED' ? 'animate-pulse ring-4 ring-indigo-600/20' : ''}`}>
                            <i className={`fa-solid ${step.icon}`}></i>
                          </div>
                          <span className={`text-[8px] font-black uppercase tracking-widest mt-4 ${active ? 'text-indigo-600 dark:text-indigo-400' : 'text-slate-400'}`}>
                            {step.label}
                          </span>
                        </div>
                      );
             ...
             <div className="relative">
                ...
                <div 
                  className="absolute left-6 top-0 w-1 bg-indigo-600 rounded-full transition-all duration-1000 ease-out shadow-[0_0_10px_rgba(79,70,229,0.5)]"
                     ...
                     return (
                       <div key={step.label} className={`flex items-start space-x-10 transition-all duration-500 ${!isCompleted && !isCurrent ? 'opacity-30' : 'opacity-100'}`}>
                          <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-xl shadow-xl transition-all duration-500 ${
                            ...
                            isCurrent ? 'bg-white dark:bg-slate-800 text-indigo-600 border-2 border-indigo-600 ring-4 ring-indigo-600/10' : 
                            'bg-slate-100 dark:bg-slate-800 text-slate-400'
                          }`}>
                             <i className={`fa-solid ${step.icon} ${isCurrent && trackingOrder.status !== 'DELIVERED' ? 'animate-bounce' : ''}`}></i>
                          </div>
                          <div>
                             <h4 className={`text-xl font-black tracking-tight ${isCurrent ? 'text-indigo-600 dark:text-indigo-400' : 'text-slate-900 dark:text-white'}`}>
                               {step.label}
                             </h4>
                             ...
                             {isCurrent && trackingOrder.status !== 'DELIVERED' && (
                               <div className="mt-4 p-4 bg-indigo-50 dark:bg-indigo-950/20 rounded-2xl border border-indigo-100 dark:border-white/5 animate-in slide-in-from-left-2">
                                  <p className="text-[10px] font-black text-indigo-600 dark:text-indigo-400 uppercase tracking-widest mb-1">Live Feed</p>
      ...
      {userOrders.length > 0 ? (
        <div className="space-y-12">
          {userOrders.map(order => {
            ...
            return (
              <div key={order.id} className="bg-white dark:bg-slate-900 rounded-[3.5rem] border border-slate-100 dark:border-white/5 shadow-2xl shadow-indigo-500/5 overflow-hidden transition-all duration-300 group">
                <div className="bg-slate-50/50 dark:bg-slate-800/20 px-10 py-8 flex flex-wrap justify-between items-center gap-8 border-b border-slate-50 dark:border-white/5">
                   <div className="flex items-center space-x-12">
                      ...
                      <div>
                        ...
                        <p className="text-sm font-black text-indigo-600 dark:text-indigo-400 uppercase tracking-tighter">₹{order.total.toLocaleString('en-IN')}</p>
                      </div>
                   ...
                   <div className="flex flex-col items-end">
                      ...
                      <div className="bg-slate-950 text-white px-4 py-1.5 rounded-xl text-[10px] font-black tracking-widest border border-white/10 group-hover:bg-indigo-600 transition-colors">
                        #{order.id.split('-')[1]}
                      </div>
                   </div>
                </div>
                ...
                <div className="p-10 space-y-12">
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-8">
                    <div className="flex items-center space-x-4">
                       <div className={`w-3 h-3 rounded-full shadow-[0_0_15px_rgba(79,70,229,0.5)] ${order.status === 'DELIVERED' ? 'bg-green-500' : 'bg-indigo-600 animate-pulse'}`}></div>
                    ...
                    <div className="flex-1 max-w-lg space-y-4">
                      <div className="relative h-2 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden">
                        <div 
                          className={`h-full bg-indigo-600 transition-all duration-1000 ease-out ${order.status === 'DELIVERED' ? 'bg-green-600' : ''}`}
                          style={{ width: `${progressPercent}%` }}
                        ></div>
                      </div>
                      ...
                      <div key={item.id} className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 group/item">
                        <div className="flex items-start space-x-8">
                          ...
                          <div className="space-y-1">
                            <Link to={`/product/${item.id}`} className="text-xl font-black text-slate-950 dark:text-white hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors tracking-tight">{item.name}</Link>
                            <p className="text-slate-400 dark:text-slate-500 text-[10px] font-black uppercase tracking-widest">{item.category}</p>
                            <p className="text-indigo-600/80 dark:text-indigo-400/80 text-xs font-bold mt-2">Quantity: {item.quantity} Units</p>
                          </div>
                        </div>
                        <div className="flex items-center space-x-4 self-end sm:self-center">
                           <button 
                            onClick={() => handleReorder([item])}
                            className="bg-indigo-600 text-white px-8 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-xl shadow-indigo-200 dark:shadow-none hover:bg-indigo-700 transition-all active:scale-95"
                           >
                            Re-Order
                           </button>
                           <button 
                ...
                <div className="bg-indigo-50/30 dark:bg-indigo-950/20 px-10 py-6 flex flex-wrap justify-between items-center gap-6">
                   <div className="flex items-center text-[10px] font-black text-indigo-600/60 dark:text-indigo-400/60 uppercase tracking-[0.3em]">
                      <i className="fa-solid fa-clock-rotate-left mr-3"></i>
                      Updated 5 Minutes Ago
                   </div>
                   <button 
                    onClick={() => setTrackingOrder(order)}
                    className="text-[10px] font-black text-slate-900 dark:text-white uppercase tracking-widest hover:text-indigo-600 flex items-center group"
                   >
                      Track Console 
        ...
        <div className="bg-white dark:bg-slate-900 p-24 rounded-[4rem] border border-indigo-50 dark:border-white/5 text-center shadow-[0_40px_100px_rgba(79,70,229,0.05)] transition-all duration-300">
          <div className="w-32 h-32 bg-indigo-50 dark:bg-indigo-950/30 rounded-[2.5rem] flex items-center justify-center mx-auto mb-10 shadow-inner">
            <i className="fa-solid fa-box-open text-6xl text-indigo-200 dark:text-indigo-900"></i>
          </div>
          ...
          <Link to="/shop" className="inline-block bg-indigo-600 text-white px-14 py-5 rounded-[2rem] font-black text-lg hover:shadow-2xl hover:bg-indigo-700 transition-all active:scale-95">
            Discover Collection
          </Link>
        </div>
      )}
