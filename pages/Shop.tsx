...
const Shop: React.FC = () => {
  ...
  return (
    <div className="space-y-12 pb-24">
      <div className="bg-indigo-700 dark:bg-slate-950 rounded-[4rem] p-16 text-white relative overflow-hidden shadow-2xl border border-indigo-800 dark:border-white/5 transition-all duration-500">
      ...
      <div className="flex flex-col lg:flex-row gap-16">
        <aside className="lg:w-80 space-y-12">
            ...
            <div className="space-y-3">
              <button 
                ...
                className={`w-full text-left px-6 py-4 rounded-2xl font-black text-sm transition-all flex items-center justify-between group ${
                  selectedCategory === 'All' ? 'bg-indigo-600 text-white shadow-xl shadow-indigo-500/20' : 'bg-white dark:bg-slate-900 border border-slate-100 dark:border-white/5 text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800'
                }`}
              >
              ...
              {CATEGORIES.map(cat => (
                <button 
                  ...
                  className={`w-full text-left px-6 py-4 rounded-2xl font-black text-sm transition-all flex items-center justify-between group ${
                    selectedCategory === cat.name ? 'bg-indigo-600 text-white shadow-xl shadow-indigo-500/20' : 'bg-white dark:bg-slate-900 border border-slate-100 dark:border-white/5 text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800'
                  }`}
                >
          ...
          <div className="bg-slate-950 p-8 rounded-[3rem] text-white relative overflow-hidden group border border-white/5">
             <div className="relative z-10">
                <p className="text-indigo-500 font-black text-[10px] uppercase tracking-widest mb-2">Member Special</p>
                ...
                <Link to="/signup" className="inline-block bg-white text-slate-950 px-6 py-3 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-indigo-600 hover:text-white transition-all">Join Club</Link>
             </div>
        ...
        <div className="flex-1">
          <div className="flex items-center justify-between mb-12 border-b border-slate-100 dark:border-white/5 pb-8">
            <p className="text-slate-500 dark:text-slate-500 font-bold text-sm tracking-widest uppercase">Showing <span className="text-indigo-600 font-black">{filteredProducts.length}</span> Objects of Desire</p>
            <div className="flex items-center text-xs font-black text-slate-400 uppercase tracking-widest">
              Sort: 
              <select 
                value={sortBy}
                onChange={(e) => setSortBy(e.target.value as any)}
                className="bg-transparent text-slate-900 dark:text-white ml-2 cursor-pointer border-b-2 border-indigo-600 pb-1 outline-none"
              >
          ...
          {filteredProducts.length > 0 ? (
            ...
            <div className="text-center py-40 bg-white dark:bg-slate-950 rounded-[4rem] border border-dashed border-slate-200 dark:border-white/10 shadow-inner">
               <div className="w-24 h-24 bg-indigo-50 dark:bg-indigo-950/20 rounded-full flex items-center justify-center mx-auto mb-8">
                  <i className="fa-solid fa-ghost text-5xl text-indigo-200 dark:text-indigo-900"></i>
               </div>
