  ...
  const checkConnectionDetails = async () => {
    if (!isOnline) {
      try {
        ...
        if (!res.ok) setConnectionError(`Server returned ${res.status}`);
      } catch (e) {
        setConnectionError("Backend server (port 5000) is unreachable. Is 'node server.js' running?");
      }
    } else {
    ...
    <div className="space-y-12 pb-20 max-w-7xl mx-auto">
      <div className="flex flex-col md:flex-row md:justify-between md:items-end space-y-4">
        <div>
          <div className="flex items-center space-x-3 mb-2">
            <span className="bg-red-600 text-white text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-widest">Admin Control Center</span>
          </div>
          <h1 className="text-5xl font-black text-slate-900 dark:text-white tracking-tighter">Inventory Console.</h1>
          <p className="text-slate-400 font-bold mt-2">Manage your elite catalog, track stock, and optimize logistics.</p>
        </div>
        <button 
        ...
        >
          <i className="fa-solid fa-plus-circle mr-3"></i> Add New Product
        </button>
      </div>

      {/* Modern Stats Grid */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
        <div className="bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] border border-red-50 dark:border-white/5 shadow-xl shadow-red-500/5 transition-all">
          <div className="w-12 h-12 bg-green-50 dark:bg-green-950/30 text-green-600 dark:text-green-400 rounded-xl flex items-center justify-center mb-6 shadow-inner">
        ...
        <div className="bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] border border-red-50 dark:border-white/5 shadow-xl shadow-red-500/5 transition-all">
          <div className="w-12 h-12 bg-red-50 dark:bg-red-950/30 text-red-600 dark:text-red-400 rounded-xl flex items-center justify-center mb-6 shadow-inner">
             <i className="fa-solid fa-box-open text-xl"></i>
          </div>
          <span className="text-slate-400 text-[10px] font-black uppercase tracking-widest block mb-1">Total Orders</span>
          <p className="text-4xl font-black text-slate-900 dark:text-white tracking-tighter">{orders.length}</p>
        </div>

        {/* Data Source Card with Troubleshooting */}
        <div className="bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] border border-red-50 dark:border-white/5 shadow-xl shadow-red-500/5 transition-all">
          <div className={`w-12 h-12 ${isOnline ? 'bg-green-50 dark:bg-green-950/30 text-green-600' : 'bg-yellow-50 dark:bg-yellow-950/30 text-yellow-600'} rounded-xl flex items-center justify-center mb-6 shadow-inner`}>
             <i className={`fa-solid ${isOnline ? 'fa-database' : 'fa-triangle-exclamation'} text-xl`}></i>
          </div>
          <span className="text-slate-400 text-[10px] font-black uppercase tracking-widest block mb-1">Data Source</span>
          <p className={`text-4xl font-black tracking-tighter ${isOnline ? 'text-green-600' : 'text-yellow-600'}`}>
            {isOnline ? 'Atlas' : 'Local'}
          </p>
          <div className="mt-4 p-3 rounded-xl bg-slate-50 dark:bg-black/20 border border-slate-100 dark:border-white/5">
            <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest leading-relaxed">
              {isOnline 
                ? 'System connected to MongoDB Cloud.' 
                : connectionError || 'Checking network status...'}
            </p>
            {!isOnline && (
              <button 
                onClick={() => window.location.reload()}
                className="mt-3 text-[8px] font-black text-red-600 uppercase border border-red-200 px-2 py-1 rounded-md hover:bg-red-600 hover:text-white transition-all"
              >
                Retry Link
              </button>
            )}
        ...
        <div className="bg-slate-950 p-8 rounded-[2.5rem] shadow-2xl shadow-slate-950/20 text-white relative overflow-hidden group">
          <i className="fa-solid fa-bolt-lightning absolute -right-4 -bottom-4 text-8xl opacity-10 group-hover:scale-110 transition-transform"></i>
          <span className="text-red-600 text-[10px] font-black uppercase tracking-widest block mb-4 flex items-center">
             Crimson AI Insight
            {loadingInsights && <i className="fa-solid fa-circle-notch fa-spin ml-2"></i>}
          </span>
          <p className="text-sm leading-snug italic font-medium line-clamp-3">
            {insights || "Synchronizing business intelligence data..."}
          </p>
          <button onClick={fetchInsights} className="mt-4 text-[9px] font-black uppercase tracking-widest underline opacity-70 hover:opacity-100 transition-opacity">Regenerate Analysis</button>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-[3rem] border border-red-50 dark:border-white/5 shadow-2xl shadow-red-500/5 overflow-hidden">
        <div className="p-8 border-b border-red-50 dark:border-white/5 bg-slate-50/50 dark:bg-slate-800/50 flex justify-between items-center">
          ...
          <span className="bg-red-100 dark:bg-red-950/40 text-red-600 dark:text-red-400 px-3 py-1 rounded-lg text-[10px] font-black uppercase">{products.length} Items Listed</span>
        </div>
          ...
          <div className="bg-white dark:bg-slate-900 w-full max-w-2xl rounded-[3rem] shadow-2xl p-12 max-h-[90vh] overflow-y-auto custom-scrollbar border border-red-50 dark:border-white/5">
            <div className="flex justify-between items-center mb-10">
              <div className="flex items-center space-x-3">
                 ...
                 <h3 className="text-3xl font-black text-slate-900 dark:text-white tracking-tighter">{editingProduct ? 'Update Listing' : 'New Masterpiece'}</h3>
              </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-2">
                  ...
                  <input 
                    ...
                    className="w-full bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white border-2 border-transparent rounded-2xl py-4 px-6 outline-none focus:ring-4 focus:ring-red-500/10 focus:border-red-600 transition-all text-lg font-bold"
                  />
                </div>
                <div className="space-y-2">
                  ...
                  <select 
                    ...
                    className="w-full bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white border-2 border-transparent rounded-2xl py-4 px-6 outline-none focus:ring-4 focus:ring-red-500/10 focus:border-red-600 transition-all text-lg font-bold cursor-pointer"
                  >
                ...
                <div className="space-y-2">
                  ...
                  <input 
                    ...
                    className="w-full bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white border-2 border-transparent rounded-2xl py-4 px-6 outline-none text-lg font-bold"
                  />
                </div>
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest ml-1">Units In Stock</label>
                  <input 
                    type="number" 
                    required
              ...
              <div className="space-y-2">
                ...
                <textarea 
                  ...
                  className="w-full bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white border-2 border-transparent rounded-2xl py-4 px-6 outline-none text-lg font-bold resize-none"
                ></textarea>
              </div>
              <button 
                type="submit"
